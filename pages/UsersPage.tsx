...
const UsersPage: React.FC = () => {
  ...
  const getOAuthIcon = (provider?: "google" | "github") => {
    if (provider === "google") {
      return (
        <GoogleIcon className="h-5 w-5 text-gray-500" title="Google OAuth" />
      );
    }
    if (provider === "github") {
  ...
  const getTierBadge = (tier: string) => {
    ...
    return (
      <Badge variant={colors[tier as keyof typeof colors] as any}>
        {tier.toUpperCase()}
      </Badge>
    );
  };
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    alert(
      "Note: Users can only be created through OAuth login (Google/GitHub). This feature is for demo purposes only."
    );
    ...
    <ProtectedRoute requiredRole="admin">
      <div className="space-y-6">
              ...
              <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                  {users.map((user) => (
                    <tr
                      key={user.id}
                      className="bg-white border-b dark:bg-card-dark dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700"
                    >
        ...
        {showAddModal && (
          ...
          >
            <div
              className="bg-white dark:bg-card-dark rounded-lg p-6 w-full max-w-md"
              ...
              <form onSubmit={handleAddUser} className="space-y-4">
                ...
                <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3">
                  <p className="text-sm text-yellow-800 dark:text-yellow-200">
                    <strong>Note:</strong> Users can only be created through
                    OAuth login. This is for demo purposes.
                  </p>
                </div>
